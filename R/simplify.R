...
#'
#' When `est = "vae"`, the automated covariate selection searches
...
#' @param est The estimation method that will be used with the simplified
#'   data (as given to `nlmixr2est::nlmixr()`).  Some estimation methods read
#'   data columns beyond the model's own covariates, and those columns are
#'   kept, too (currently only `est = "vae"`; see Details).  `NULL` keeps
#'   only the standard and model covariate columns.
#' @param control The estimation control object matching `est`, when its
#'   settings affect which extra columns the method reads (for `est = "vae"`:
#'   `shapes`, `covCenterType`, `covCenter`, and `catCutoff`).
