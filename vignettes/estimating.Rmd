...
Compartment initial conditions can be set with either the natural nlmixr2 DSL
form `cmt(0) <- value` directly inside a `model({...})` block, or the
manual workaround `cmt(initial) <- value`. Both are accepted; both produce
the same fit. `tar_nlmixr()` and `tar_nlmixr_multimodel()` quietly rewrite
`cmt(0)` to `cmt(initial)` in the function binding so that `targets`' static
analysis can walk the model body, and restore `cmt(0)` before nlmixr2 sees
