...
A logical vector the same length as `model_list` indicating if the model
is self-referential to another model in the list

## Functions

- `tar_nlmixr_multimodel_has_self_reference_single()`: A helper function
  to look at each call for each model separately
