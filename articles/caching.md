...
# Renaming a `tar_nlmixr_multimodel()` entry:
#   only re-runs the cheap combining target that collects the fits into a
#   named list. The name is the description that `_fit_simple` announces
#   when it runs, and it is deliberately excluded from that target's hash.
```
