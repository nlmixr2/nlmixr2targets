...
lets you declare several candidate models for the **same** dataset in
one call. Each model gets its own simplify/fit/relabel chain, but the
...
This works because
[`tar_nlmixr_multimodel()`](https://nlmixr2.github.io/nlmixr2targets/reference/tar_nlmixr_multimodel.md)
returns a target whose value is a named list of fits.
...
Behind the scenes, `nlmixr2targets` rewrites `candidate_fits[["Base"]]`
to a dependency on the base model’s `_fit_simple` target. Iteratively
resolving these references is what makes within-list piping work without
forcing the entire pipeline to re-run when one model changes.

If you write a circular reference (A piping from B and B piping from A)
the function errors out at construction time with a clear message.

## Tips for large model libraries

- **Group models by data flavor.** Each call to
  [`tar_nlmixr_multimodel()`](https://nlmixr2.github.io/nlmixr2targets/reference/tar_nlmixr_multimodel.md)
  is scoped to one dataset. If you have several datasets, use one call
  per dataset (with distinct `name`s) and combine the resulting lists at
  the plan level.
- **Use
  [`tar_outdated()`](https://docs.ropensci.org/targets/reference/tar_outdated.html)
  aggressively** when iterating. Cosmetic edits (labels, metadata) do
  not invalidate `_fit_simple` thanks to the strip-restore behaviour.
  See
  [`vignette("caching", package = "nlmixr2targets")`](https://nlmixr2.github.io/nlmixr2targets/articles/caching.md).
